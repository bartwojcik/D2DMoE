    ...
    for exp_name in exp_names:
        ...
        for exp_id in exp_ids:
            ...
            if x_key_name is not None:
                assert x_key_name in set(
                    in_stats[run_name]
                ), f"{x_key_name} not in stats of run {run_name}"
                x_scores.append(in_stats[run_name][x_key_name])
            if y_key_name is not None:
                assert y_key_name in set(
                    in_stats[run_name]
                ), f"{y_key_name} not in stats of run {run_name}"
                y_scores.append(in_stats[run_name][y_key_name])
        if (len(y_scores) == 0 or len(x_scores) == 0) and (
            x_key_name is not None and y_key_name is not None
        ):
            continue
        ...
        if x_key_name is not None and len(x_scores) > 0:
            ...
            x_std, x_mean = torch.std_mean(x_scores.double(), dim=0)
        ...
        if y_key_name is not None and len(y_scores) > 0:
            ...
            y_std, y_mean = torch.std_mean(y_scores, dim=0)
