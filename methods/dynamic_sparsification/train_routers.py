...
from architectures.moe.moefication import MoeficationMoE, add_routers
...
def setup_for_training(args, tc):
    ...
    tc.captured_layer_name_map = get_captured_layer_name_map(unwrapped_model, tc.moe_modules, args.dsti_router_labels_layer)
    tc.saved_inputs, input_handles = add_save_inputs_hook(unwrapped_model, tc.moe_modules.keys())
    tc.saved_output_norms, output_handles = add_save_output_norm_hook(unwrapped_model, tc.captured_layer_name_map.values(), ord=args.dsti_router_labels_norm, dim=-1)
    ...
    while tc.state.current_batch <= tc.last_batch:
        ...
        for moe_name, moe in tc.moe_modules.items():
            ...
            with torch.no_grad():
                ...
                router_label = captured_output_norm.view(captured_output_norm.size(0), input.size(0), input.size(1))
        ...
        if tc.accelerator.is_main_process:
            tc.writer.add_scalar('Train/Average loss', loss.item(), global_step=tc.state.current_batch)
...
def final_eval(args, tc):
    if not tc.final_path.exists():
        ...
        final_losses = []
        final_flops = []
        for tau in args.dsti_tau_to_test:
            if tc.accelerator.is_main_process:
            ...
            if tc.accelerator.is_main_process:
                ...
                tc.writer.add_scalar(f'Eval with tau={tau}/Model FLOPs', total_average_flops,
                                     global_step=tc.state.current_batch)
        # TODO add also evaluation with topk (or perhaps in a different, separate step / script?)
        if tc.accelerator.is_main_process:
            ...
            final_results['hyperparam_values'] = args.dsti_tau_to_test
