...
from architectures.moe.moe_layers import MOE_IMPL_MAP, ExecuteAllExperts, ModuleBatchedExperts, MoELayer
from architectures.moe.moe_models import (
    moe_attention_forward,
    moe_bert_attention_forward,
    moe_bert_layer_forward,
    moe_bert_main_forward,
    moe_gpt_block_forward,
    moe_gpt_main_forward,
    moe_vit_block_forward,
    moe_vit_encoder_forward,
    moe_vit_main_forward,
)
...
class MoeficationMoE(MoELayer):
    ...
    def __init__(
        ...
        self.bias = bias
        # instantiate experts
        self.experts = MOE_IMPL_MAP[experts_class](
            dim=hidden_dim,
...
def replace_layer_with_moe(model, moefied_module_name, num_experts=None, expert_size=None, experts_class="module"):
    ...
    if num_experts is not None:
        assert d_ff % num_experts == 0, "d_ff has to be divisible by the number of experts"
        expert_size = d_ff // num_experts
    elif expert_size is not None:
        assert d_ff % expert_size == 0, "d_ff has to be divisible by the expert size"
        num_experts = d_ff // expert_size
    add_residual = True if isinstance(original_module, ResidualMLP) else False
    moe_layer = MoeficationMoE(
        moefied_module_name,
        hidden_dim,
        num_experts,
        w1.bias is not None,
        ...
        activation,
        experts_class,
        add_residual_connection=add_residual,
    )
    ...
    if isinstance(original_module, BertLayer):
...
def replace_with_moes(
    original_model: nn.Module,
    num_experts: int = None,
    expert_size: int = None,
    experts_class="module",
    module_filter_contition=None,
):
    assert num_experts is not None or expert_size is not None, "either num_experts or expert_size must be passed"
    assert not (
        num_experts is not None and expert_size is not None
    ), "num_experts and expert_size cannot be both passed"
    original_model.eval()
    # avoid deepcopying as it crashes jit.trace for model with forward wrappers
    model = deepcopy(original_model)
    modules_to_moeify = find_module_names(model, module_filter_contition)
    ...
    if isinstance(model, (VisionTransformer, TorchvisionViT)):
        for i in range(len(model.encoder.layers)):
            ...
            if isinstance(model.encoder.layers[i].self_attention, CustomMultiheadAttention):
                model.encoder.layers[i].self_attention.forward = partial(
                    moe_attention_forward, model.encoder.layers[i].self_attention
                )
        ...
        for i in range(len(model.transformer.h)):
            ...
            if isinstance(model.transformer.h[i].attn, CustomMultiheadAttention):
                model.encoder.layers[i].attn.forward = partial(moe_attention_forward, model.transformer.h[i].attn)
        ...
        for i in range(len(model.bert.encoder.layer)):
            ...
            if isinstance(model.bert.encoder.layer[i].attention, CustomMultiheadAttention):
                ...
                model.bert.encoder.layer[i].attention.forward = partial(moe_bert_attention_forward, model.bert.encoder.layer[i].attention)
...
def param_clustering_split(ffn, moe_layer):
    ...
    labels = KMeansConstrained(n_clusters=num_experts, size_min=expert_size, size_max=expert_size).fit_predict(
        w1_normalized.detach().cpu().numpy()
    )
    ...
    if isinstance(moe_layer.experts, ModuleBatchedExperts):
        ...
        with torch.no_grad():
            ...
            for neuron_index, expert_index in enumerate(labels):
                ...
                moe_layer.experts.e[expert_index][2].weight[:, expert_neuron_index].copy_(w2.weight[:, neuron_index])
...
class MoeficationRouter(nn.Sequential):
    def __init__(
        self, hidden_dim, num_experts, width=128, depth=2, bias=False, activation="tanh", output_activation="identity"
    ):
        layers = []
        if depth == 1:
...
def add_routers(model, router_args):
    ...
    for moe_module_name in moe_module_names:
        ...
        moe_module.router = MoeficationRouter(moe_module.hidden_dim, moe_module.num_experts, **router_args)
