...
from architectures.early_exits.heads import HEAD_TYPES, TransformerHead, CNNHead, SwinHead
        ...
        while True:
            ...
            if i in self._place_at:
                if issubclass(head_class, CNNHead):
                    in_size = x.size(1)
                elif issubclass(head_class, TransformerHead):
                    in_size = x.size(2)
                elif issubclass(head_class, SwinHead):
                    in_size = x.size(-1)
                in_size_list.append(in_size)
            i += 1
